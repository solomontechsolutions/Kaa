...
import { currentActor, requireActor } from "@/lib/operators/session";
...
/**
 * One FieldOps submission, reviewed from inside Kaa Operators.
 *
 * The twin of `/field/submissions/[id]` — same data, same
 * `ReviewActions` component, same `/api/fieldops/submissions/[id]/review`
 * endpoint underneath. What differs is who is looking at it: this page only
 * ever renders for a signed-in Kaa operator (`requireActor` here comes from
 * `lib/operators/session`, not FieldOps'), and FieldOps' own portal is not
 * reachable to reach this same decision a second way.
 */
export default async function OperatorsFieldOpsSubmissionDetail(props: { params: Promise<{ id: string }> }) {
  const { id } = await props.params;
